...
def about(subject):
    ...
    return lambda paragraph: not set(subject).isdisjoint(split(remove_punctuation(lower(paragraph))))
    # END PROBLEM 2
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if not typed and not source or limit < 0:
        return 0
...
def time_per_word(words, timestamps_per_player):
    ...
    for p in timestamps_per_player:
        times.append([p[i + 1] - p[i] for i in range(len(p) - 1)])
